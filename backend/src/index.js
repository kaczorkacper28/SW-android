...
async function discordGet(path) {
  ...
  if (!response.ok) { const error = new Error(data?.message || 'Discord API error'); error.status=response.status; error.details=data; throw error; }
  return data;
}

function normalizeRoleName(value) {
  return String(value || '').replace(/ł/g,'l').replace(/Ł/g,'L').normalize('NFD').replace(/[\u0300-\u036f]/g,' ').replace(/[^a-zA-Z0-9 ]/g,' ').replace(/\s+/g,' ').trim().toLowerCase();
}
...
function findRank(roleNames) {
  ...
  for (const rule of rules) if (roles.some(role => rule.words.some(word => role===word || role.includes(word)))) return rule.rank;
  return null;
}

function hasCitizenRole(roleNames) {
  return roleNames.map(normalizeRoleName).some(role => role==='obywatel' || role.startsWith('obywatel ') || role.endsWith(' obywatel') || role.includes(' obywatel '));
}

function hasSwRole(roleNames, rank) {
  if (rank) return true;
  return roleNames.map(normalizeRoleName).some(role => role==='sw' || role.startsWith('sw ') || role.endsWith(' sw') || role.includes(' sw ') || role.includes('sluzba wiezienna') || role.includes('funkcjonariusz') || role.includes('funkcjonarius') || role.includes('kandydat'));
}

function hasAnyNonCitizenRole(roleNames) {
  return roleNames.some(role => { const normalized=normalizeRoleName(role); return normalized && normalized!=='@everyone' && !normalized.includes('obywatel'); });
}

function isTechnicalRole(name) {
  const r=normalizeRoleName(name);
  if (!r || r==='@everyone') return true;
  const technical=['bot','admin','administrator','moderator','mod','staff','owner','wlasciciel','zarzad','zarzadca','developer','deweloper','technik','support','pomoc','obywatel','booster','nitro','mute','muted','everyone','pracownik sw'];
  return technical.some(x => r===x || r.startsWith(x+' ') || r.endsWith(' '+x));
}

function getActualSwRole(roleObjects, rank) {
  if (rank) return rank;
  const candidates=roleObjects.filter(r => r && !isTechnicalRole(r.name) && !normalizeRoleName(r.name).includes('obywatel')).sort((a,b)=>Number(b.position||0)-Number(a.position||0));
...
app.get('/api/discord/channels',async(req,res)=>{
  try {
    ...
    const visible=channels.filter(c=>c.type===0||c.type===5).map(c=>({id:c.id,name:c.name,type:c.type,parentId:c.parent_id||null,position:c.position||0}));
    res.json({channels:visible});
  } catch(error) { res.status(error.status||500).json(discordError(error.status||500,error.details)); }
});
...
app.post('/api/broadcaster/send',async(req,res)=>{
  ...
  try {
    ...
    broadcastHistory.unshift(entry); if(broadcastHistory.length>100) broadcastHistory.pop();
...
app.get('/api/officers',async(req,res)=>{
  try {
    if(!process.env.DISCORD_BOT_TOKEN) return res.status(503).json({error:'DISCORD_BOT_TOKEN_NOT_CONFIGURED',message:'Na backendzie nie ustawiono DISCORD_BOT_TOKEN.'});
    const discordRoles=await discordGet(`/guilds/${GUILD_ID}/roles`);
    const roleMap=new Map(discordRoles.map(role=>[role.id,role]));
    const allMembers=[]; let after=null;
    for(let page=0;page<100;page++){
      const url=new URL(`https://discord.com/api/v10/guilds/${GUILD_ID}/members`); url.searchParams.set('limit','1000'); if(after) url.searchParams.set('after',after);
      const response=await fetch(url,{headers:{Authorization:`Bot ${process.env.DISCORD_BOT_TOKEN}`}}); const members=await response.json();
      if(!response.ok) return res.status(response.status).json(discordError(response.status,members));
      if(!Array.isArray(members)||members.length===0) break;
      allMembers.push(...members); after=members[members.length-1]?.user?.id||null; if(members.length<1000||!after) break;
    }
    const parsed=allMembers.filter(member=>member.user&&!member.user.bot).map(member=>{
      ...
      const rank=findRank(roleNames), citizen=hasCitizenRole(roleNames), sw=hasSwRole(roleNames,rank), fallback=hasAnyNonCitizenRole(roleNames), actualRole=getActualSwRole(roleObjects,rank);
      return {member,roleObjects,roleNames,rank,actualRole,citizen,sw,fallback};
    });
    const officers=parsed.filter(item=>!item.citizen).filter(item=>item.sw||item.fallback).map(item=>{
      const m=item.member,realRank=item.actualRole;
      return {id:m.user.id,username:m.user.username,displayName:m.nick||m.user.global_name||m.user.username,avatar:m.user.avatar?`https://cdn.discordapp.com/avatars/${m.user.id}/${m.user.avatar}.png`:null,roles:m.roles||[],roleNames:item.roleNames,rank:realRank,rankRole:realRank,allRoles:item.roleNames,status:'Aktywny',joinedAt:m.joined_at||null};
    });
    const rankOrder=new Map(ranks.map((rank,index)=>[rank,index]));
    officers.sort((a,b)=>{const ar=rankOrder.has(a.rank)?rankOrder.get(a.rank):-1; const br=rankOrder.has(b.rank)?rankOrder.get(b.rank):-1; if(ar!==br) return br-ar; return a.displayName.localeCompare(b.displayName,'pl',{sensitivity:'base'});});
    const citizensExcluded=parsed.filter(item=>item.citizen).length;
    res.json({guildId:GUILD_ID,count:officers.length,officers,meta:{membersScanned:allMembers.length,citizensExcluded,officersWithRank:officers.filter(o=>o.rank!=='Brak przypisanej rangi SW').length,officersWithoutRank:officers.filter(o=>o.rank==='Brak przypisanej rangi SW').length}});
  } catch(error) { console.error('Officers backend error:',error); res.status(error.status||500).json(discordError(error.status||500,error.details)); }
});
