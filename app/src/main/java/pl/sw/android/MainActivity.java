...
public class MainActivity extends Activity {
    ...
    private void base(String title) {
        ...
        root.addView(label("🛡️  SŁUŻBA WIĘZIENNA", 24, true));
        root.addView(label(title, 21, true));
    ...
    private void home() {
        ...
        root.addView(o); root.addView(bc); root.addView(sc); root.addView(r); root.addView(d); root.addView(p);
    ...
    private void broadcaster() {
        ...
        mention.setAdapter(new ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, new String[]{"Bez wzmianki", "@everyone", "@here"}));
        root.addView(title); root.addView(content); root.addView(channel); root.addView(color); root.addView(mention);
        Button send = menu("📤  WYŚLIJ KOMUNIKAT"); root.addView(send);
        TextView result = label("", 15, false); root.addView(result);
        Button history = menu("📜  Historia komunikatów"); root.addView(history); back();
    ...
    private void send(String title, String content, String channel, String color, int mi, TextView result) {
        if (content.trim().isEmpty() || channel.trim().isEmpty()) { result.setText("❌ Wpisz treść i ID kanału."); return; }
        executor.execute(() -> {
            try {
                ...
                j.put("channelId", channel.trim()); j.put("title", title); j.put("content", content);
            ...
            } catch (Exception e) { runOnUiThread(() -> result.setText("❌ Błąd wysyłania: " + e.getMessage())); }
        });
    }

    private void history() {
        base("📜 Historia Broadcastera");
        TextView s = label("⏳ Pobieram historię...", 16, false); root.addView(s); back();
        executor.execute(() -> {
            try {
                ...
                StringBuilder z = new StringBuilder(); String q; while ((q = r.readLine()) != null) z.append(q);
                JSONObject j = new JSONObject(z.toString()); JSONArray a = j.optJSONArray("history");
                runOnUiThread(() -> {
            ...
            } catch (Exception e) { runOnUiThread(() -> s.setText("❌ Nie można pobrać historii.\n" + e.getMessage())); }
        });
    }
